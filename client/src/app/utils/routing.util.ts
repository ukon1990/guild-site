...
export class RoutingUtil {
  ...
  static getCurrentRoute(path: string): MenuItem | undefined {
    if (!this.list.length) {
      RoutingUtil.getMenu();
    }
    if (!path || !this.list) {
  ...
  private static queryForMatchingParts(list: MenuItem[], parts: string[], partIndex: number = 1): MenuItem | undefined {
  ...
  private static getIsHidden({isHidden}: TitledRoute): boolean {
    switch (isHidden) {
      case RouteHiddenFlag.ADMIN_ONLY:
        return !this.isAdmin;
      case RouteHiddenFlag.ONLY_IN_DEVELOP:
        return false; // environment.production;
      case RouteHiddenFlag.IS_REGISTERED:
        return true; // SharedService.events.isUserSet.value;
      case RouteHiddenFlag.IS_NOT_REGISTERED:
        return false; // !SharedService.events.isUserSet.value;
      case RouteHiddenFlag.ALWAYS:
        return true;
      default:
        return false;
    }
  ...
  private static getMenuItem({title, path, children, canActivate, isHidden}: TitledRoute, parent?: MenuItem) {
    ...
    const menuItem: MenuItem = new MenuItem(
      ...
      routerLinkFullPath,
      undefined,
      isHidden === RouteHiddenFlag.ONLY_IN_DEVELOP
    );
    if (children) {
      children.forEach(child => {
        if (child.title && !this.getIsHidden(child) && this.canActivate(child.canActivate)) { // Use Can activate also
          ...
          childMenuItem.onlyInDevelop = child.isHidden === RouteHiddenFlag.ONLY_IN_DEVELOP;
          childMenuItems.push(childMenuItem);
        }
      });
    }
    this.map.set(routerLinkFullPath, menuItem);
    return menuItem;
  }
